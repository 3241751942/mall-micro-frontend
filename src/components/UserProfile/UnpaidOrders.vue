<template>
  <div>
    ...
    <div v-else-if="orders.length === 0">暂无待支付订单</div>
    <div v-else>
      <div v-for="order in orders" :key="order.orderNo" class="order-card">
        ...
        <div class="order-footer">
          ...
          <div class="order-buttons">
            <el-button type="primary" size="small" @click="$emit('pay', order)">支付</el-button>
            <el-button type="danger" size="small" @click="$emit('cancel', order.orderNo)">取消订单</el-button>
...
const emit = defineEmits(['pay', 'cancel', 'reorder', 'addToCart', 'pageChange'])

// 默认占位图（请确保 public/images/placeholder.webp 存在）
